...
bam.summ <- read_table("results/results_cleaning/bam.metrics.tsv",
                       col_names=FALSE) %>% 
  filter(grepl("^results/bam/", X1) |
         grepl("^[0-9]", X1)) %>% 
...
bam.names <- read_table("results/results_cleaning/bam.metrics.tsv",
                        col_names=FALSE)[7,] %>% 
...
Nfree.counts.summ <- read_table("results/results_cleaning/Rsubread.stdout.txt", 
                   col_names = FALSE) %>% 
